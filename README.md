📍 Campus Navigator: KC Model School & College

<p align="center">
<img src="https://img.shields.io/badge/Founder-Khan%20Jariff-059669?style=for-the-badge&logo=github" />
<img src="https://img.shields.io/badge/Firm-Glitched%20Technologies-fbbf24?style=for-the-badge" />
<img src="https://img.shields.io/badge/Status-In%20Development-orange?style=for-the-badge" />
</p>

Campus Navigator is a high-performance digital infrastructure tool designed to streamline navigation for students and faculty at KC Model School & College. Developed by Glitched Technologies, this application provides real-time access to classroom locations, faculty directories, and campus facilities across 8 levels (Basement to 7th Floor).
📲 Download & Access
Platform	Link Status
🌐 Web Version	🚀 Launch on Vercel Live
🤖 Android (APK)	📥 Download Latest Release (Coming Soon)
🚀 Core Features

    🔍 Omni-Filter Search: Instant filtering by Class name, Section (e.g., SB-1), Form Teacher, or Room number.

    📂 Dynamic Floor Directory: Detailed layouts for all levels, from KG and Primary to Secondary and College sections.

    📞 Faculty Connect: Integrated directory with one-tap contact options for class representatives and teachers.

    🎨 Glitched UI/UX: A bespoke, high-contrast dark-mode interface optimized for mobile performance and readability.

    📱 Cross-Platform: Built with React and TypeScript, ready for Vercel web deployment and Android APK distribution.

🛠️ Technical Stack

    ⚛️ Frontend: React 18 with TypeScript for type-safe development.

    🎨 Styling: Tailwind CSS for a utility-first, responsive "Glitched" aesthetic.

    🧩 Icons: Lucide React for consistent, scalable vector graphics.

    🧠 State: React Context API for handling global campus data and navigation.

    ⚡ Backend: Supabase for data persistence and Vercel for automated CI/CD.

📦 Installation & Setup

To run this project locally, ensure you have Node.js installed.
🛠️ Clone the repository


    git clone https://github.com/your-username/campus-navigator.git

📁 Navigate to the directory


    cd campus-navigator

⚡ Install dependencies


    npm install

🚀 Start the development server

    
    npm run dev

⚡ The "3-Line Boom" Update

To push new updates live to the web version:


    git add .
    git commit -m "feat: updated floor detail filtering"
    git push origin main

🗺️ Roadmap

    [x] 🎨 Initial UI/UX Design System (Glitched Tech Style)

    [x] 📍 Functional Floor Navigation & Room Directory

    [ ] 🍔 Facilities Section (Canteen, Lab, Library)

    [ ] 🐛 Bug Reporting & Suggestion Hub

    [ ] 🤖 Android APK Release via Capacitor

🛡️ Leadership & Credits

    Lead Engineer & Founder: Khan Jariff Al Naseeb

    The Team: Glitched Technologies

📩 Bug Reports & Suggestions

Found a bug or have a feature idea? Please open an Issue or submit a Pull Request. We are actively improving the navigator based on student feedback.📍 Campus Navigator: KC Model School & College

<p align="center">
<img src="https://img.shields.io/badge/Founder-Khan%20Jariff-059669?style=for-the-badge&logo=github" />
<img src="https://img.shields.io/badge/Firm-Glitched%20Technologies-fbbf24?style=for-the-badge" />
<img src="https://img.shields.io/badge/Status-In%20Development-orange?style=for-the-badge" />
</p>

Campus Navigator is a high-performance digital infrastructure tool designed to streamline navigation for students and faculty at KC Model School & College. Developed by Glitched Technologies, this application provides real-time access to classroom locations, faculty directories, and campus facilities across 8 levels (Basement to 7th Floor).
📲 Download & Access
Platform	Link	Status
🌐 Web Version	Launch on Vercel	Live
🤖 Android (APK)	Download Latest Release	Coming Soon
🚀 Core Features

    🔍 Omni-Filter Search: Instant filtering by Class name, Section (e.g., SB-1), Form Teacher, or Room number.

    📂 Dynamic Floor Directory: Detailed layouts for all levels, from KG and Primary to Secondary and College sections.

    📞 Faculty Connect: Integrated directory with one-tap contact options for class representatives and teachers.

    🎨 Glitched UI/UX: A bespoke, high-contrast dark-mode interface optimized for mobile performance and readability.

    📱 Cross-Platform: Built with React and TypeScript, ready for Vercel web deployment and Android APK distribution.

🛠️ Technical Stack

    ⚛️ Frontend: React 18 with TypeScript for type-safe development.

    🎨 Styling: Tailwind CSS for a utility-first, responsive "Glitched" aesthetic.

    🧩 Icons: Lucide React for consistent, scalable vector graphics.

    🧠 State: React Context API for handling global campus data and navigation.

    ⚡ Backend: Supabase for data persistence and Vercel for automated CI/CD.

📦 Installation & Deployment
Local Development

To run this project locally, ensure you have Node.js installed.
Bash

# 🛠️ Clone the repository
git clone https://github.com/your-username/campus-navigator.git

# 📁 Navigate to the directory
cd campus-navigator

# ⚡ Install dependencies
npm install

# 🚀 Start the development server
npm run dev

⚡ The "3-Line Boom" Update

To push new updates live to the web version:
Bash

git add .
git commit -m "feat: updated floor detail filtering"
git push origin main

🗺️ Roadmap

    [x] 🎨 Initial UI/UX Design System (Glitched Tech Style)

    [x] 📍 Functional Floor Navigation & Room Directory

    [ ] 🍔 Facilities Section (Canteen, Lab, Library)

    [ ] 🐛 Bug Reporting & Suggestion Hub

    [ ] 🤖 Android APK Release via Capacitor

🛡️ Leadership & Credits

    Lead Engineer & Founder: Khan Jariff Al Naseeb

    The Team: Glitched Technologies

📩 Bug Reports & Suggestions

Found a bug or have a feature idea? Please open an Issue or submit a Pull Request. We are actively improving the navigator based on student feedback.
